 ...
 *   3. Run: node import-ga-views.js <path-to-csv>
 *
 * The script maps GA page paths to Blowfish document IDs:
 *   /posts/my-article/ → views_posts-my-article
 */
...
const csvPath = process.argv.find(arg => !arg.startsWith('--') && !arg.includes('node') && !arg.includes('import-ga-views'));
if (!csvPath) {
  console.error('Usage: node import-ga-views.js <path-to-csv> [options]');
  console.error('');
...
const LANGUAGE_PREFIXES = ['en', 'fr', 'zh-cn', 'es', 'de', 'it', 'ja', 'pt-pt'];

/**
 * Strip language prefix from a URL path
 * /pt-pt/posts/my-article/ → /posts/my-article/
 * /posts/my-article/ → /posts/my-article/ (unchanged)
 */
 ...
 *   GA path: /posts/202401-evolution-ai/
 *   Hugo file: posts/202401-evolution-ai/index.md
 *   Firestore ID: views_posts-202401-evolution-ai-index.md
 */
function pathToDocId(pagePath) {
  ...
  // For section pages like /posts/, the file is posts/_index.md
  // For article pages like /posts/my-article/, the file is posts/my-article/index.md
  // We can tell them apart: section pages don't have a second path segment after the section

  const parts = cleanPath.split('/');
  if (parts.length === 1) {
    // Section page like "posts" → posts/_index.md
    cleanPath = `${cleanPath}-_index.md`;
  } else {
    // Article page like "posts/my-article" → posts/my-article/index.md → posts-my-article-index.md
      ...
      .on('data', (row) => {
        ...
        if (pagePath && views) {
          ...
          if (cleanPath.startsWith('/') && !isNaN(viewCount) && viewCount > 0) {
            ...
            // Include posts, homepage, and other sections you care about
            const validPrefixes = ['/posts/', '/projects/', '/about/', '/resume/', '/music/'];
            const isValidPath = normalizedPath === '/' ||
                               validPrefixes.some(p => normalizedPath.startsWith(p)) ||
                               validPrefixes.some(p => normalizedPath === p.slice(0, -1)); // /posts without trailing slash
