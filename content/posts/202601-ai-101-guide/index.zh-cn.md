...
[Docker Model Runner](https://docs.docker.com/desktop/features/model-runner/) 将模型推理带入 Docker 生态系统。如果你的基础设施是基于 Docker 的，DMR 自然适合你的工作流程。

```bash
# 像任何 Docker 镜像一样拉取并运行模型
...
- OpenAI 兼容的 API 端点
- GPU 支持和优化
- 熟悉的 Docker 工作流程用于模型部署

相对于 Ollama 的优势不一定是技术上的——而是运维上的。如果你的团队已经用 Docker 容器思考，你的部署管道是基于 Docker 的，DMR 意味着少学习和维护一个工具。

{{< alert icon="lightbulb" >}}
**Ollama vs DMR：** 两者都工作得很好。Ollama 有更大的社区和模型库。DMR 与基于 Docker 的基础设施集成得更好。许多团队两者都用——Ollama 用于快速本地实验，DMR 用于基于 Docker 的部署。
{{< /alert >}}
...
{{< alert icon="lightbulb" >}}
**为什么要关心这个？** MCP 正在成为关键基础设施。Anthropic、OpenAI、Microsoft 和 Google 都宣布了支持。Docker 有一个 MCP 目录，下载量超过一百万次。如果你正在构建 AI 应用，理解 MCP 很快将与理解 REST API 一样重要。
{{< /alert >}}
...
Docker 已经全力投入 MCP，推出了 [Docker MCP Toolkit](https://docs.docker.com/ai/mcp-catalog-and-toolkit/)——一个预构建 MCP 服务器的目录和运行它们的工具。
...
为什么这很重要：你可以组合现有的 MCP 服务器来给你的智能体提供能力，而不是从头构建集成。需要你的智能体访问 GitHub 和 Slack？拉取那些 MCP 服务器，配置凭据，完成。

```bash
# 示例：从 Docker 目录运行 MCP 服务器
docker run -d mcp/github --token $GITHUB_TOKEN
```

该工具包还提供用于管理 MCP 服务器、处理身份验证和调试智能体-工具交互的实用程序。

### cagent

[cagent](https://github.com/docker/cagent) 是 Docker 的智能体框架——一个用于构建可以通过 MCP 使用工具的 AI 智能体的开源工具包。

与编程智能体（专注于编写代码）不同，cagent 是一个通用框架，用于创建与系统交互的智能体。它处理编排、工具发现和执行循环，这样你可以专注于定义你的智能体应该完成什么。
...
如果你正在构建需要与外部系统（数据库、API、文件）交互的自定义智能体，cagent 为你提供了坚实的基础，而不是从头构建。
...
{{< alert icon="circle-info" >}}
**模式正在趋同。** 大多数成功的智能体使用这些技术的某种组合。像 LangChain、LlamaIndex 和 cagent 这样的框架编码了这些模式，所以你不必从头实现它们。
{{< /alert >}}
...
- **尝试本地模型。** 安装 [Ollama](https://ollama.ai) 或 [Docker Model Runner](https://docs.docker.com/desktop/features/model-runner/)，在你的笔记本电脑上运行 Llama 或 Qwen。这出奇地简单。
- **探索智能体。** 查看 LangChain、LlamaIndex、CrewAI 或 cagent 等框架来构建智能体系统。
- **学习 MCP。** [官方文档](https://modelcontextprotocol.io)很扎实。尝试用 Docker 的 MCP Toolkit 在本地运行一些 MCP 服务器。
...
- [Docker AI](https://docs.docker.com/ai/)——DMR、MCP Toolkit 和 cagent
