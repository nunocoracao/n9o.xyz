...
{{< alert icon="lightbulb" >}}
**なぜ重要なのか？** MCPは重要なインフラストラクチャになりつつあります。Anthropic、OpenAI、Microsoft、Googleはすべてサポートを発表しています。AIアプリケーションを構築している場合、MCPを理解することはまもなくREST APIを理解することと同じくらい重要になるでしょう。
{{< /alert >}}
...
{{< alert icon="circle-info" >}}
**パターンは収束しています。** ほとんどの成功したエージェントはこれらのテクニックの組み合わせを使用しています。LangChain、LlamaIndexなどのフレームワークはこれらのパターンをエンコードしているので、ゼロから実装する必要はありません。
{{< /alert >}}

### スキル

**スキル**は、エージェントができることを拡張する再利用可能で専門化されたプロンプトです。エージェントにプラグインできる「エキスパートモード」と考えてください - コードレビュー用のスキル、ドキュメント作成用のスキル、セキュリティ脆弱性分析用のスキルなど。

ツール（何かを*実行する*関数）とは異なり、スキルはエージェントが*どのように*考え、応答するかを形作る指示です。ツールはAPIを呼び出します。スキルはエージェントに「Xについて質問されたら、このようにアプローチし、これらの要素を考慮し、このように応答をフォーマットしてください」と伝えます。
...
1. **システムプロンプト：** 最も一般的なアプローチ。スキルはエージェントの基本指示の一部となり、常にアクティブです。
2. **ユーザーメッセージプレフィックス：** ユーザーリクエストに動的に前置されます。タスク固有のスキルに便利です。
3. **ツール説明：** スキルはツール定義に埋め込むことができ、エージェントが特定のツールをどのように使用するかをガイドします。
4. **MCPプロンプト：** MCPサーバーはスキルを「プロンプト」として公開できます - クライアントが発見して呼び出せる再利用可能なテンプレートです。
...
{{< alert icon="lightbulb" >}}
**スキルエコシステムは発展中です。** エージェントがより普及するにつれて、スキルマーケットプレイスや標準化されたスキルフォーマットが登場することが予想されます。MCPのプロンプト機能はこれに向けた初期のステップです - 異なるエージェント実装間で発見、共有、呼び出しできるスキルです。
{{< /alert >}}
...
- **ローカルモデルを試す。** [Ollama](https://ollama.ai)をインストールして、ノートパソコンでLlamaまたはQwenを実行します。驚くほど簡単です。
- **エージェントを探索する。** エージェントシステムを構築するためにLangChain、LlamaIndex、CrewAIなどのフレームワークをチェックしてください。
- **MCPを学ぶ。** [公式ドキュメント](https://modelcontextprotocol.io)はしっかりしています。ローカルでいくつかのMCPサーバーを実行してみてください。
