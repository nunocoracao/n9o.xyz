...
ここが、ドナの結末によって譲れなくなった部分です。Fridayのメインドライバーは、OpenAIの5.6ファミリーのコストバランス型ティアであるGPT-5.6 Terraです。Terraに届かないときはGPT-5.5に落ち、30分ごとのハートビートのような、フロンティアモデルではお金の無駄になる定型作業もGPT-5.5が担います。そしてOpenAI自体の調子が悪い日には、同じ箱の専用LXCコンテナで動く [Ollama](https://ollama.com) 経由のQwen3 8Bに着地します。能力は劣りますが、常時稼働で、誰にもその条件を変えられません。
...
<svg viewBox="0 0 720 152" xmlns="http://www.w3.org/2000/svg" font-family="ui-sans-serif,system-ui,-apple-system,sans-serif" style="width:100%;height:auto" role="img" aria-label="モデルのフォールバックチェーン: メインドライバーはGPT-5.6 Terra、次はハートビートも担当するGPT-5.5、その次はOllama経由のローカルQwen3 8Bで常時稼働。ベンチには、クレジットがあるときのClaude Opus 4.8とFable 5、そして素早いローカル作業用のLlama 3.2 3B。">
  ...
  <text x="32" y="86" font-size="11" fill="currentColor" fill-opacity="0.55">OpenAI・従量課金</text>
  ...
  <text x="271" y="86" font-size="11" fill="currentColor" fill-opacity="0.55">OpenAI・従量課金</text>
...
もう、どのモデルプロバイダーも単一障害点ではありません。私が眠っている間にどこかがルールを変えても、Fridayはしばらく遅く、少し賢くなくなるだけで、消えはしません。これは裏返しのモデル信仰ではなく、ドナの物語から導かれる、ただのエンジニアリング上の結論です。
